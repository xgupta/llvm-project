...
public:
  ...
  enum ChecksumKind {
    ...
    // ChecksumInfo in an std::optional, but to preserve Bitcode compatibility
    // the 0
...
public:
  /// Basic Type decimal info, used to record std::optional dwarf decimal
  /// attributes such as picture_string, digit_counts, decimal_sign,
  /// decimal_scale all or some can be absent and std::optional depending on
  /// encoding type.
  struct DecimalInfo {
...
private:
  ...
  getImpl(LLVMContext &Context, DIScope *Scope, StringRef Name,
    ...
    return getImpl(Context, Scope, getCanonicalMDString(Context, Name),
                   ...
                   dyn_cast_or_null<Metadata>(RcFrameBaseExpr), Storage,
                   ShouldCreate);
  }
  static DISubprogram *
  ...
  TempDISubprogram cloneImpl() const {
    return getTemporary(
        ...
        getStaticLinkExpr(), getRcFrameBaseExpr());
  }
...
public:
  ...
  /// Prepend \p DIExpr with a deref and offset operation and std::optionally
  /// turn it into a stack value or/and an entry value.
  static DIExpression *prepend(const DIExpression *Expr, uint8_t Flags,
                               int64_t Offset = 0);

  /// Prepend \p DIExpr with the given opcodes and std::optionally turn it into
  /// a stack value.
