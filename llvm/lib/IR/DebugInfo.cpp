...
static DILocalVariable::DIVarFlags
pack_into_DIVarFlags(bool IsLocatorDesc) {
...
LLVMMetadataRef
LLVMDIBuilderCreateArrayType2(LLVMDIBuilderRef Builder, uint64_t Size,
                             uint32_t AlignInBits, LLVMMetadataRef Ty,
                             LLVMMetadataRef *Subscripts,
                             unsigned NumSubscripts,
			     LLVMBool isVarString,
			     const char *Name, size_t NameLen) {
  auto Subs = unwrap(Builder)->getOrCreateArray({unwrap(Subscripts),
                                                 NumSubscripts});
  return wrap(unwrap(Builder)->createArrayType(Size, AlignInBits,
...
LLVMMetadataRef LLVMDIBuilderCreateDecimalType(
  ...
  return wrap(unwrap(Builder)->createBasicType(
      { Name, NameLen }, { PicString, PicLen }, SizeInBits, Encoding,
      DigitCount, DecimalSign, Scale, map_from_llvmDIFlags(Flags)));
}
...
LLVMMetadataRef LLVMDIBuilderCreateDynamicType(LLVMDIBuilderRef Builder,
  ...
  return wrap(unwrap(Builder)->createDynamicType(
      ...
      unwrapDI<DIExpression>(Allocated)));
}
...
LLVMMetadataRef LLVMDIBuilderCreateExpressionWithRef(LLVMDIBuilderRef Builder,
  ...
  return wrap(unwrap(Builder)->createExpression(
      ArrayRef<int64_t>(Addr, Length), {unwrap(Ref), RefLength}));
}
...
LLVMMetadataRef LLVMDIBuilderCreateAutoVariable2(
    LLVMDIBuilderRef Builder, LLVMMetadataRef Scope, const char *Name,
    size_t NameLen, LLVMMetadataRef File, unsigned LineNo, unsigned LexicalScope,
    LLVMMetadataRef Ty, LLVMBool AlwaysPreserve, LLVMDIFlags Flags,
    uint32_t AlignInBits, LLVMBool IsLocatorDesc) {
  return wrap(unwrap(Builder)->createAutoVariable2(
                  unwrap<DIScope>(Scope), {Name, NameLen}, unwrap<DIFile>(File),
                  LineNo, LexicalScope, unwrap<DIType>(Ty), AlwaysPreserve,
                  map_from_llvmDIFlags(Flags),
                  pack_into_DIVarFlags(IsLocatorDesc), AlignInBits));
}

LLVMMetadataRef LLVMDIBuilderCreateParameterVariable2(
    LLVMDIBuilderRef Builder, LLVMMetadataRef Scope, const char *Name,
    size_t NameLen, unsigned ArgNo, unsigned LexicalScope, 
    LLVMMetadataRef File, unsigned LineNo, LLVMMetadataRef Ty,
    LLVMBool AlwaysPreserve, LLVMDIFlags Flags, LLVMBool IsLocatorDesc) {
  return wrap(unwrap(Builder)->createParameterVariable2(
                  ...
                  map_from_llvmDIFlags(Flags),
                  pack_into_DIVarFlags(IsLocatorDesc)));
}
