...
template <> struct MDNodeKeyImpl<DIBasicType> {
  ...
  MDNodeKeyImpl(unsigned Tag, MDString *Name, MDString *PictureString,
      ...
      : Tag(Tag), Name(Name), PictureString(PictureString),
        ...
        NumExtraInhabitants(NumExtraInhabitants), Flags(Flags), DecimalAttrInfo(DecimalAttrInfo) {}
  MDNodeKeyImpl(const DIBasicType *N)
      : Tag(N->getTag()), Name(N->getRawName()),
        PictureString(N->getRawPictureString()), SizeInBits(N->getSizeInBits()),
        AlignInBits(N->getAlignInBits()), Encoding(N->getEncoding()),
        NumExtraInhabitants(N->getNumExtraInhabitants()), Flags(N->getFlags()), DecimalAttrInfo(N->getDecimalInfo()) {}
  ...
  bool isKeyOf(const DIBasicType *RHS) const {
    return Tag == RHS->getTag() && Name == RHS->getRawName() &&
           ...
           Encoding == RHS->getEncoding() && NumExtraInhabitants == RHS->getNumExtraInhabitants() 
           && Flags == RHS->getFlags() &&
...
template <> struct MDNodeKeyImpl<DICompositeType> {
  ...
  MDNodeKeyImpl(unsigned Tag, MDString *Name, Metadata *File, unsigned Line,
      ...
      : Tag(Tag), Name(Name), File(File), Line(Line), Scope(Scope),
        BaseType(BaseType), SizeInBits(SizeInBits), OffsetInBits(OffsetInBits),
        AlignInBits(AlignInBits), Flags(Flags), VFlags(VFlags), Elements(Elements),
