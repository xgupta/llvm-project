      ...
      if (is_dynamic) {
        ...
        if (loc_expr.IsValid()) {
          ...
          if (!loc_expr.Evaluate(&exe_ctx, nullptr, loclist_base_load_addr,
                                 nullptr, &obj_addr)) {
            m_error.FromErrorString("dynamic variable location read error");
            return m_error.Success();
          }
