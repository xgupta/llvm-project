...
static bool SearchCompilerTypeForMemberWithName(CompilerType *comp_type,
  ...
  llvm::Expected<uint32_t> expected_count =
      comp_type->GetNumChildren(true, nullptr);
  if (!expected_count) {
    llvm::consumeError(expected_count.takeError());
    return false;
  }
  uint32_t total_count = *expected_count;
...
CobolInterpreter::EvaluateStatement(const lldb_private::CobolASTStmt *stmt) {
  ...
  switch (stmt->GetKind()) {
  default:
    m_error.FromErrorStringWithFormat("%s node not supported",
                                     stmt->GetKindName());
    break;
...
lldb::ValueObjectSP CobolInterpreter::GetLevel88(llvm::StringRef var_name,
  ...
  auto parent_expr = "const char* arg = \"" + var_name.str() +
                     "\"; (char *) rc_cob_get_level88_parent(arg)";
...
lldb::ValueObjectSP CobolInterpreter::VisitIdent(const CobolASTIdent *ident) {
  ...
  if (m_frame) {
    ...
    if (var_name[0] == '$') {
      m_error.Clear();
      m_error.FromErrorString("Consistent var lookup not implemented yet");
      return nullptr;
    }
    ...
    if (!result) {
      ...
      if (!target) {
        m_error.Clear();
        m_error.FromErrorString("No target");
        return nullptr;
      }
  ...
  if (!result)
    m_error.FromErrorStringWithFormat("Unknown variable %s",
                                     var_name.str().c_str());
  return result;
}
...
ValueObjectSP CobolInterpreter::VisitBasicLit(const CobolASTBasicLit *expr) {
  ...
  default:
    m_error.FromErrorStringWithFormat("Non-Const lexical type for %s",
  ...
  case CobolLexer::LIT_INTEGER:
    ...
    if (value_string.getAsInteger(0, iValue)) {
      m_error.FromErrorStringWithFormat("integer conversion error %s",
                                       value_string.str().c_str());
      return nullptr;
    }
  ...
  case CobolLexer::LIT_FLOAT:
    if (value_string.getAsDouble(dValue)) {
      m_error.FromErrorStringWithFormat("double conversion error %s",
                                       value_string.str().c_str());
      return nullptr;
    }
...
CobolInterpreter::VisitSelectorExpr(const CobolASTSelectorExpr *expr) {
  ...
  if (target) {
    ...
    if (!result)
      m_error.FromErrorStringWithFormat("Unknown child %s", field.AsCString());
    return result;
  }
  ...
  if (auto ref = llvm::dyn_cast<CobolASTRefModifierExpr>(expr->GetExpr())) {
    if (auto ident = llvm::dyn_cast<const CobolASTIdent>(ref->GetExpr())) {
      ...
      if (!result) {
        m_error.FromErrorStringWithFormat("Unknown child %s",
...
ValueObjectSP CobolInterpreter::GetElementAtIndex(lldb::ValueObjectSP var,
  ...
  if ((start < 1) || (start > max_elem)) {
    m_error.FromErrorStringWithFormat("Out of bound index: %d.", start);
    return nullptr;
  }
...
uint32_t CobolInterpreter::GetUIntFromValueObjectSP(ValueObjectSP var) {
  ...
  if (index_string.getAsInteger(10, index)) {
    m_error.FromErrorStringWithFormat("ref modifier invalid index %s.",
...
CobolInterpreter::VisitFuncCallExpr(const CobolASTFuncCallExpr *expr) {
  ...
  if (expr->getTotalNumParams() != 1) {
    m_error.FromErrorString("wrong number of params for sizeof operator.");
    return nullptr;
  }
