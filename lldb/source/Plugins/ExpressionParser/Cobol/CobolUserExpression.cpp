...
CobolInterpreter::VisitRefModExpr(const CobolASTRefModifierExpr *expr) {
  ...
  ValueObjectSP CobolInterpreter::VisitFuncCallExpr(
    ...
    if (expr->getTotalNumParams() != 1) {
      m_error = Status::FromErrorString(
          "wrong number of params for sizeof operator.");
      return nullptr;
    }
    ...
    uint8_t addr_size = target->GetArchitecture().GetAddressByteSize();
    DataEncoder enc(buffer->GetBytes(), sizeof(data_size), byte_order,
                    addr_size);
  ...
  ValueObjectSP CobolInterpreter::VisitCompareExpr(
      const CobolASTCompareExpr *expr) {
    Value valF(Scalar(0));
  ...
  ValueObjectSP CobolInterpreter::VisitAssignmentExpr(
      const CobolASTAssignmentExpr *expr) {
  ...
  bool CobolUserExpression::Parse(
  ...
  lldb::ExpressionResults CobolUserExpression::DoExecute(
      DiagnosticManager & diagnostic_manager, ExecutionContext & exe_ctx,
      const EvaluateExpressionOptions &options,
