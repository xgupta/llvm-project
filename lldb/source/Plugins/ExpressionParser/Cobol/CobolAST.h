...
public:
  ...
  const char *GetKindName() const override { return "Index Expression"; }

  size_t GetNumberOfIndices() const { return m_indices.size(); }
...
private:
  ...
  const CobolASTIndexExpr &operator=(const CobolASTIndexExpr &) = delete;
};
