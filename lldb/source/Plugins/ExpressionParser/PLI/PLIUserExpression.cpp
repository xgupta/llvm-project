...
PLIInterpreter::VisitRefModExpr(const PLIASTRefModifierExpr *expr) {
  ValueObjectSP var = EvaluateExpr(expr->GetExpr());
  if (!var) {
    m_error.FromErrorString("variable not found.");
    return nullptr;
  }
  ...
  if (!var->GetCompilerType().IsArrayType(&elem_type, &max_elem,
                                          &is_incomplete)) {
    m_error.FromErrorStringWithFormat("variable %s is not an array.",
  ...
  if (!start_var) {
    m_error.FromErrorString("ref modifier invalid indexes.");
    return nullptr;
  }
  ...
  if (start >= max_elem) {
    m_error.FromErrorStringWithFormat("out of bound index: %d.", start + 1);
    return nullptr;
  }
