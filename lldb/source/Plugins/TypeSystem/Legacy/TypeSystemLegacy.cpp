...
Format TypeSystemLegacy::GetFormat(opaque_compiler_type_t type) {
  ...
  // Temporary handle bit type for failing test TestDataFormatterStdVBool.py
  ConstString type_name_cs = GetTypeName(type, true);
  std::string type_name = type_name_cs.AsCString();
  if (type_name.find("bit") == 0) {
...
TypeSystemLegacy::GetNumChildren(lldb::opaque_compiler_type_t type,
