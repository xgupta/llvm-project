...
uint32_t TypeSystemLegacy::GetNumChildren(opaque_compiler_type_t type,
...
CompilerType TypeSystemLegacy::GetArrayElementType(opaque_compiler_type_t type,
  ...
  if (array) {
    if (stride) {
      if (auto byte_size = array->GetElementType().GetByteSize(nullptr))
        *stride = *byte_size;
    }
...
TypeSystemSP TypeSystemLegacy::CreateInstance(LanguageType language,
