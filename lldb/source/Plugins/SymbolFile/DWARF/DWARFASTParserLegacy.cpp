  ...
  if (die) {
    ...
    if (!type_ptr) {
      ...
      case DW_TAG_base_type: {
        ...
        type_sp = dwarf->MakeType(die.GetID(), type_name_const_str,
                                  (bit_size + 7) / 8, nullptr, LLDB_INVALID_UID,
        ...
        if (element_type) {
          ...
          if (array_element_type.GetCompleteType()) {
            ...
            if (array_info && array_info->element_orders.size() > 0) {
              ...
              for (auto pos = array_info->element_orders.rbegin(); pos != end;
                ...
                if (num_elements)
                  array_element_bit_stride *= num_elements;
                       ...
                       array_info->element_orders_exp.size() > 0) {
              auto end = array_info->element_orders_exp.rend();
              for (auto pos = array_info->element_orders_exp.rbegin();
                   pos != end; ++pos) {
              ...
              compiler_type = m_ast.CreateArrayType(
                  type_name_const_str, empty_name, array_element_type,
                  (size_t)0, isVarString);

            type_sp = dwarf->MakeType(
                die.GetID(), empty_name, array_element_bit_stride / 8, nullptr,
      ...
      case DW_TAG_structure_type: {
        ...
        type_sp =
            dwarf->MakeType(die.GetID(), type_name_const_str, byte_size,
      ...
      case DW_TAG_dynamic_type: {
        ...
        type_sp = dwarf->MakeType(die.GetID(), empty_name, 0, nullptr,
      ...
      case DW_TAG_subroutine_type: {
        ...
        type_sp = dwarf->MakeType(die.GetID(), type_name_const_str, 0, nullptr,
                                  LLDB_INVALID_UID, Type::eEncodingIsUID, &decl,
                                  compiler_type, Type::ResolveState::Full);
...
lldb_private::Function *DWARFASTParserLegacy::ParseFunctionFromDWARF(
  ...
  if (die.GetDIENamesAndRanges(name, mangled, unused_func_ranges, decl_file,
    ...
    // Find the lowest and highest addresses in the ranges
    lldb::addr_t lowest_func_addr = LLDB_INVALID_ADDRESS;
    lldb::addr_t highest_func_addr = 0;

    for (const auto &range : unused_func_ranges) {
      if (lowest_func_addr == LLDB_INVALID_ADDRESS ||
          range.LowPC < lowest_func_addr)
        lowest_func_addr = range.LowPC;
      if (range.HighPC > highest_func_addr)
        highest_func_addr = range.HighPC;
    }

    if (lowest_func_addr != LLDB_INVALID_ADDRESS &&
        lowest_func_addr <= highest_func_addr) {
      ModuleSP module_sp(die.GetModule());
      llvm::DWARFAddressRange func_range(lowest_func_addr, highest_func_addr);

      // Use the range start address directly
      if (module_sp) {
        // func_range.LowPC =
        // module_sp->GetSectionList()->ResolveFileAddress(lowest_func_addr);
      }

      if (func_range.LowPC != LLDB_INVALID_ADDRESS) {
        Mangled func_name;
        ...
        if (dwarf->FixupAddress(func_range.LowPC)) {
          const user_id_t func_user_id = die.GetID();
          func_sp =
              std::make_shared<Function>(&comp_unit, func_user_id, func_user_id,
                                         func_name, func_type, func_ranges);
...
lldb_private::Function *DWARFASTParserLegacy::ParseFunctionFromDWARF(
    lldb_private::CompileUnit &comp_unit,
    const lldb_private::plugin::dwarf::DWARFDIE &die,
    const lldb_private::AddressRanges func_ranges) {
  if (die.Tag() != DW_TAG_subprogram)
    return nullptr;
