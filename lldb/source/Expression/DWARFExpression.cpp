...
//  std::vector<Value> stack;
  ...
  while (opcodes.ValidOffset(offset)) {
    ...
    case DW_OP_RC_byte_swap:
      if (stack.empty()) {
        return llvm::createStringError("Expression stack needs at least 1 item for DW_OP_RC_byte_swap.");
      } else {
      ...
      if (stack.empty()) {
        ...
        if (stack.back().GetValueType() == Value::ValueType::FileAddress) {
          auto file_addr =
              stack.back().GetScalar().ULongLong(LLDB_INVALID_ADDRESS);
          if (!module_sp) {
              return llvm::createStringError("need module to resolve file address for DW_OP_deref");
          }
          Address so_addr;
          if (!module_sp->ResolveFileAddress(file_addr, so_addr)) {
              return llvm::createStringError("failed to resolve file address in module");
